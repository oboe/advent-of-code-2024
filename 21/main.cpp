...
int64_t getShortest(const std::string& presses, std::vector<std::string> pad,
                    const int level);
...
const std::vector<std::string> keypad{{"X^A"}, {"<v>"}};

const std::vector<std::string> numpad{{"789"}, {"456"}, {"123"}, {"X0A"}};
...
std::string getMpStr(const int x, const int y, const int xx, const int yy,
                     const int level, const bool isKeypad) {
  std::string ans{(isKeypad) ? "keypad-" : "numpad-"};
  ans += std::to_string(x) + "-" + std::to_string(y) + "-";
  ans += std::to_string(xx) + "-" + std::to_string(yy) + "-" +
         std::to_string(level);
  return ans;
}

int64_t getShortestPath(const int x, const int y, const int xx, const int yy,
                        const int level, const bool isKeypad = true) {
  ...
  while (!dq.empty()) {
    ...
    if (v.x == xx && v.y == yy) {
      int64_t rec =
          getShortest(v.presses + "A", keypad, (isKeypad) ? level - 1 : level);
...
int64_t getShortest(const std::string& presses,
                    const std::vector<std::string> pad, const int level) {
